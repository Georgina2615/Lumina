      ...
      {!loading && !error && (
        <div className="mt-7 grid gap-4 sm:grid-cols-2">
          {services.map((service) => {
            ...
            return (
              <button aria-pressed={selected} className={`relative rounded-2xl border p-4 text-left transition duration-300 hover:-translate-y-0.5 hover:shadow-md active:scale-[0.99] motion-reduce:transform-none sm:p-5 ${selected ? 'border-secondary bg-status-pending/10 shadow-sm ring-1 ring-secondary/20' : 'border-surface-hover bg-background'}`} key={service.id} onClick={() => onSelect(service.id)} type="button">
