    ...
    <section className="relative overflow-hidden px-5 py-20 sm:px-8 lg:px-12 lg:py-28">
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="relative mt-12 grid gap-4 lg:grid-cols-3">
          <div aria-hidden="true" className="absolute left-[16%] right-[16%] top-10 hidden h-px bg-gradient-to-r from-transparent via-status-pending/60 to-transparent lg:block" />
          {experienceSteps.map(({ description, icon: Icon, number, title }, index) => (
            <Reveal className="relative" delay={index * 100} key={number}>
              <article className="group h-full rounded-3xl border border-surface-hover bg-surface/80 p-6 shadow-sm backdrop-blur transition duration-300 hover:-translate-y-1 hover:border-status-pending/50 hover:shadow-xl hover:shadow-primary/5 active:scale-[0.99] motion-reduce:transform-none sm:p-7">
                <div className="flex items-center justify-between">
                  <span className="grid h-12 w-12 place-items-center rounded-2xl bg-primary text-status-pending shadow-lg shadow-primary/10 transition duration-300 group-hover:-rotate-3 group-hover:scale-105 motion-reduce:transform-none">
