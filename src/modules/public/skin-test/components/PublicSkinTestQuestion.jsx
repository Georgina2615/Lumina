    ...
    <section aria-labelledby="skin-question-title">
      ...
      <div aria-hidden="true" className="mt-4 flex gap-1.5">
        {Array.from({ length: test.questionsCount }, (_, index) => (
          <span className={`h-2 flex-1 rounded-full transition-colors duration-300 ${index <= test.currentIndex ? 'bg-brand-sage' : 'bg-surface-hover'}`} key={index} />
        ))}
      </div>
      ...
      <div className="mt-8 grid gap-3">
        {test.currentQuestion.options.map((option) => {
          ...
          return (
            <button aria-pressed={selected} className={`min-h-16 rounded-2xl border px-5 py-4 text-left text-sm font-medium transition duration-200 ${selected ? 'border-brand-gold bg-brand-blush/40 text-primary shadow-sm' : 'border-surface-hover bg-background text-muted hover:border-brand-gold/50 hover:text-primary'}`} key={option.id} onClick={() => test.selectOption(option.id)} type="button">
